        ...
        <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                <div className="bg-[#0B2A2F]/70 backdrop-blur-md text-white rounded-xl w-full max-w-lg overflow-hidden relative border border-white/10 shadow-xl">
                    ...
                    {item.image && (
                        <div className="w-full h-40 md:h-48 relative px-4 md:px-6 pt-4 md:pt-6">
                    ...
                    <div className="p-4 md:p-6">
                        <h2 className="text-xl md:text-2xl font-serif text-[#C8A97E] mb-2 md:mb-3">
                            {item.name}
                        </h2>
                        ...
                        <div className="mb-4 md:mb-6">
                            ...
                            <ul
                                className={`space-y-1.5 md:space-y-2 transition-all duration-300 md:h-auto ${showIngredients ? 'h-auto opacity-100' : 'h-0 opacity-0 md:opacity-100'} overflow-hidden`}
                            >
                            ...
                            <div className="flex gap-2 md:gap-3">
                                {portions.map(({ size, persons }) => (
                                    <button
                                        ...
                                        className={`flex-1 rounded-lg p-2 transition-all duration-300 ${selectedPortion === size ? 'bg-[#C8A97E] text-[#0B2A2F]' : 'bg-white/5 hover:bg-white/10 backdrop-blur-sm'}`}
                                    >
                                        <div className="text-xs md:text-sm font-medium">
                                            {size}
                                            <span
                                                className={`text-[10px] md:text-xs ml-1 ${selectedPortion === size ? 'text-[#0B2A2F]/70' : 'text-gray-400'}`}
                                            >
                        ({persons})
                      </span>
                                        </div>
